...
async def helper(message):
	...
	embed.add_field(name='```am [movie name]```', value='\tAdd a movie to the watchlist', inline=False)
	embed.add_field(name='```rm [movie name]```', value='\tRemove a movie from the watchlist', inline=False)
	embed.add_field(name='```mw [movie name]```', value='\tYour movie watchlist', inline=False)
	embed.add_field(name='```mf [movie name]```', value='\tInfo about a movie', inline=False)
	embed.add_field(name='```cl```', value='\tClears all movies from the watchlist', inline=False)
	embed.add_field(name='```trending```', value='\tDisplays 5 random movies from a list of the top 200 trending movies', inline=False)
	embed.add_field(name='```watched [movie name]```', value='\tAdd a movie into the watched movie list', inline=False)
	embed.add_field(name='```watchedrm [movie name]```', value='\tRemove a movie from the watched movie list', inline=False)
	embed.add_field(name='```wl [movie name]```', value='\tGo through watched movie list', inline=False)
	embed.add_field(name='```rw [movie name] (1-5)```', value='\tSubmit a rating of 1-5 for a movie', inline=False)
	embed.set_thumbnail( 
	    url=
